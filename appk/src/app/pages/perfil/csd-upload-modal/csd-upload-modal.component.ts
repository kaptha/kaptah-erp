...
@Component({
    selector: 'app-csd-upload-modal',
    templateUrl: './csd-upload-modal.component.html',
    styleUrls: ['./csd-upload-modal.component.css'],
    encapsulation: ViewEncapsulation.None,
    standalone: false
})
export class CsdUploadModalComponent {
  form: FormGroup;
  ...
  isLoading = false;
  modalTitle: string = 'Subir Certificado de Sello Digital (CSD)';
  userRfc: string = '';

  constructor(
    public dialogRef: MatDialogRef<CsdUploadModalComponent>,
  ...
  async onKeyFileSelected(event: any): Promise<void> {
    const file = event.target.files[0];
    if (file && file.name.toLowerCase().endsWith('.key')) {
      this.keyFile = file;
      this.keyFileName = file.name;
    } else {
      this.snackBar.open('Por favor seleccione un archivo .key válido', 'Cerrar', { duration: 3000 });
    }
  }
  ...
  async onCerFileSelected(event: any): Promise<void> {
    ...
    if (file && file.name.toLowerCase().endsWith('.cer')) {
      ...
      this.snackBar.open('Por favor seleccione un archivo .cer válido', 'Cerrar', { duration: 3000 });
    }
  }
  ...
  async onSave(): Promise<void> {
    ...
    try {
      ...
      if (certInfo.rfc && this.userRfc) {
        ...
        Sweetalert.fnc('error', 
          'No se pudo extraer el RFC del certificado. Verifica que sea un archivo CSD válido.', 
          null
        );
        return;
      }
    ...
    this.csdService.uploadCsd(
      ...
      next: (response) => {
        this.snackBar.open('CSD cargado exitosamente', 'Cerrar', { duration: 3000 });
        this.dialogRef.close(response);
      },
      error: (error) => {
        console.error('Error al cargar CSD:', error);
        this.snackBar.open(error.error?.message || 'Error al cargar el CSD', 'Cerrar', { duration: 3000 });
