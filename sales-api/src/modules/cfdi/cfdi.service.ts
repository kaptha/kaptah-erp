...
// Verificación local del sello
    ...
    const cfdi = this.cfdiRepository.create({
      ...
      subtotal: this.extractTotalFromXml(resultadoTimbrado.cfdiTimbrado, 'SubTotal') ?? this.calculateSubtotal(createDto.conceptos),
      total: this.extractTotalFromXml(resultadoTimbrado.cfdiTimbrado, 'Total') ?? this.calculateTotal(createDto.conceptos),
